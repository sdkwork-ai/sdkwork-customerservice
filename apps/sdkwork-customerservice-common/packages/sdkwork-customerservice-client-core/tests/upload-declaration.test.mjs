...
const CONSTANTS_PATH = path.join(appRoot, 'packages/sdkwork-customerservice-client-core/src/services/uploadDeclaration.ts');
...
const PREFIX_PATTERN =
  '(?:^|[^A-Za-z0-9_])export const CUSTOMERSERVICE_[A-Z0-9_]*_UPLOAD(?:_[A-Z0-9_]+)?\u005cs*=\u005cs*\u005c{';
...
test('upload declaration declares one identity per entry, allowing a per-surface source', () => {
  ...
  // This root declares one purpose once per host surface. The identity may therefore repeat,
  // but only once per surface: the surface count bounds the identity count, and a source that
  // appears twice would be a duplicate declaration rather than a second surface.
  const sources = declarations.map((entry) => entry.source);
  assert.equal(new Set(sources).size, sources.length, 'a source label is declared twice');
  assert.ok(unique.size <= sources.length, 'no identity may repeat more often than it has surfaces');
  assert.ok(unique.size >= 1, 'at least one identity is required');
});
...
test('upload declaration constants agree on one source label', () => {
  ...
  // This root is deliberately declared once per host surface, so several source labels are
  // expected; each must be a distinct surface label rather than a duplicate of another.
  assert.ok(sources.size >= 1, 'at least one source label is required');
  assert.equal(sources.size, loadDeclaration().declarations.length,
    'each declared surface must carry its own distinct source label');
});
